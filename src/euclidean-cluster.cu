...
EuclideanClusterExtractor::ObsReturn EuclideanClusterExtractor::extractClusters(GPU_Cloud_F4 pc) {
    ...
    __device__ int findNextLargestSquare(int num){
        int exp = log2(num) + 1;
        return pow(2,exp);
    }
    ...
    __global__ void findClearPathKernel(float* minXG, float* maxXG, float* minZG, int numClusters, int* leftBearing, int* rightBearing) {
        ...
        const int numThreads = findNextLargestSquare(numClusters);
        if(threadIdx.x >= numThreads) return;

        __shared__ float maxX[numThreads];
        __shared__ float minX[numThreads];
        __shared__ float minZmaxX[numThreads];
        __shared__ float minZminX[numThreads];
        ...
        if((leftLine(maxX, minZ) > -1 && rightLine(maxX, minZ) < 1) || //check if maxX is between right and left
           ...
           (leftLine(minX, minZ, maxX, minZ) || rightLine(minZ, minZ, maxX, minZ)) { //check if lines intersect line seg
            maxX[threadIdx.x] = maxX;
            minX[threadIdx.x] = minX;
            minZmaxX[threadIdx.x] = minZ;
            minZminX[threadIdx.x] = minZ;
        }
        else {
            maxX[threaIdx.x] = FLT_MIN;
            minX[threadIdx.x] = FLT_MAX;
        }

        __syncthreads();

        float minZminXL = minZ;
        float minZminXL = minZ;
        //Perform parallel reduction to find minX and maxX in path
        int aliveThreads = numThreads/2;
        while (aliveThreads > 0) {
            if (threadIdx.x < aliveThreads && localMin[threadIdx.x+aliveThreads] != -1) {
                //Check if value smaller than min
                if(minX > minX[threadIdx.x*2]) {
                    minX = minX[threadIdx.x*2];
                    minZminXL = minZminX[threadIdx.x*2];
                }
                //Check if value larger than max
                if(maxX < maxX[threadIdx.x*2]) {
                    maxX = minX[threadIdx.x*2];
                    minZmaxXL = minZmaxX[threadIdx.x*2];
                } 
                ...
                if (threadIdx.x >= (aliveThreads) / 2) {
                    maxX[threadIdx.x] = maxX;
                    minX[threadIdx.x] = minX;
                    minZmaxX[threadIdx.x] = minZmaxXL;
                    minZminX[threadIdx.x] = minZminXL;
                }
            }
            __syncthreads();
            aliveThreads /= 2;
        ...
        *bearing = (threadIdx.x == 0 && maxX != FLT_MIN) : 100 ? 0;//There is an obstacle in the path
    }
